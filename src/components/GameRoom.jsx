...
const GameRoom = ({ roomInfo, playerName, onLeave, socket }) => {
    // جلب بيانات اللعبة (حولنا القيمتين لنصوص عشان نضمن التطابق)
    const game = gamesList.find(g => String(g.id) === String(roomInfo.gameId));
    ...
    useEffect(() => {
        // بمجرد فتح الشاشة، اللاعب ينضم للغرفة الفعلية في السيرفر
        socket.emit('join-game', { roomCode: roomInfo.code, name: playerName });

        // استقبال التحديثات
        const handleUpdatePlayers = (list) => setPlayers(list);
        const handleSyncGame = (data) => {
        ...
        socket.on('sync-game', handleSyncGame);
    ...
    const submitAnswer = (e) => {
        ...
        // إرسال الإجابة مع التأكد من إرفاق الإجابة الصحيحة للسيرفر ليقارنها
        socket.emit('check-answer', { 
            roomCode: roomInfo.code, 
            answer: answer, 
            playerName: playerName,
            correctAns: questions[currentQIndex]?.correct, 
    ...
    const handleLeaveRoom = () => {
        socket.emit('leave-game', roomInfo.code);
        onLeave();
    };
        ...
        <div className="flex flex-col lg:flex-row gap-6 p-4 h-screen bg-[#050505] text-white">
            ...
            <div className="flex-1 bg-[#0A0A0A] border border-[#FF2400] rounded-3xl p-8 flex flex-col relative overflow-hidden shadow-[0_0_40px_rgba(255,36,0,0.15)]">
                ...
                {winner ? (
                    <div className="absolute inset-0 bg-[#0A0A0A] flex flex-col items-center justify-center z-50 animate-fade-in">
                        ...
                        <button onClick={handleLeaveRoom} className="relative z-10 px-12 py-4 bg-[#FF2400] text-black font-black text-2xl rounded-2xl hover:bg-white hover:scale-105 transition-all shadow-[0_0_20px_rgba(255,36,0,0.5)]">
                            العودة للوبي
                        </button>
                    </div>
                ) : (
                    <>
                        <div className="flex justify-between items-center mb-10 border-b border-gray-800 pb-6">
                            ...
                            <div className="flex gap-4 items-center">
                                ...
                                <button onClick={handleLeaveRoom} className="px-5 py-2 bg-red-900/20 text-red-500 border border-red-900/50 hover:bg-red-600 hover:text-white rounded-xl font-bold transition-all">مغادرة</button>
                            </div>
                        </div>
            ...
            <div className="w-full lg:w-96 flex flex-col gap-6">
                ...
                <div className="flex-1 bg-[#0A0A0A] p-5 rounded-3xl border border-gray-800 flex flex-col shadow-xl overflow-hidden">
                    ...
                    <div ref={chatRef} className="flex-1 overflow-y-auto flex flex-col gap-3 mb-4 pr-2">
                        {messages.map((m, i) => (
